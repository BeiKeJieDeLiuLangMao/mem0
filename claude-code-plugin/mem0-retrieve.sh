...
format_memories() {
    ...
    # 遍历记忆并格式化
    echo "$memories_json" | jq -r '.[] | "- **\(.memory // .content // .text // "...")**" ' 2>/dev/null || true
...
main() {
    ...
    if [[ ${#user_message} -lt 5 ]]; then
        log_debug "Message too short, skipping"
        exit 0
    fi
    ...
    # 搜索相关记忆
    local memories_json
    if ! memories_json=$(timeout "$TIMEOUT" search_memories "$user_message" "$user_id" "." 2>/dev/null); then
